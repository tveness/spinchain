// This simulate the Langevin dynamics on the spin chain.
// This requires modifying the original spin chain struct in a number of ways
// 1. Open boundary conditions on the energy
// 2. Dynamics now have a Landau-Lifshitz-Gilbert term on the edges of the chain
...
impl SpinChainLangevin {
    ...
    pub fn new(conf: Config, num: usize, gamma: f64) -> SpinChainLangevin {
    ...
    pub fn log(&self) {
        //They are one and the same in this case
    ...
    pub fn total_energy(&self) -> f64 {
        ...
        let mut sjs: f64 = 0.0;
        for i in 0..(s - 1) {
            sjs -= Self::sjs_energy(&self.spins[i], &self.spins[i + 1], &self.j_couple[i]);
            //            for k in 0..3 {
        ...
        //Periodic term
        sjs -= Self::sjs_energy(&self.spins[s - 1], &self.spins[0], &self.j_couple[s - 1]);
        //        for k in 0..3 {
        ...
        let mut sh: f64 = 0.0;
        for i in 0..s {
            sh += Self::sh_energy(&self.spins[i], &self.static_h[i]);
            //            for k in 0..3 {
        ...
        let h_e: [f64; 3] = self.h_ext(self.t);
        for i in 0..ss {
            sh += Self::sh_energy(&self.spins[i], &h_e);
            //            for k in 0..3 {
    ...
    fn cross(l: &[f64], r: &[f64], gamma: f64) -> [f64; 3] {
        [
            gamma * (l[1] * r[2] - r[1] * l[2]),
            gamma * (l[2] * r[0] - r[2] * l[0]),
            gamma * (l[0] * r[1] - r[0] * l[1]),
        ]
    }
    ...
    fn even_omega(&self, h: &[[f64; 3]], delta_t: f64, h_l: &[f64; 3]) -> Vec<[f64; 3]> {
        ...
        for n in 1..l {
            ...
            result.push(Self::ssh_sum(&left_s, &right_s, &h[2 * n]));
        }
    ...
    fn odd_omega(&self, h: &[[f64; 3]], delta_t: f64, h_r: &[f64; 3]) -> Vec<[f64; 3]> {
        ...
        for n in 0..l - 1 {
            ...
            result.push(Self::ssh_sum(&left_s, &right_s, &h[2 * n + 1]));
        }
...
mod tests {
    ...
    fn vec_sum() {
        ...
        let sum_v1_v2: [f64; 3] = SpinChainLangevin::sum_vec(&v1, &v2);
    ...
    fn mag_energy() {
        ...
        let s1doth: f64 = SpinChainLangevin::sh_energy(&s1, &f);
        let s2doth: f64 = SpinChainLangevin::sh_energy(&s2, &f);
        let s3doth: f64 = SpinChainLangevin::sh_energy(&s3, &f);
    ...
    fn initialise_energy() {
        ...
        for i in 0..num as usize {
            let sc: SpinChainLangevin = SpinChainLangevin::new(Config::default(), 0, 1.0);
    ...
    #[test]
    fn interactions_off() {
        let mut sc: SpinChainLangevin = SpinChainLangevin::new(Config::default(), 0, 1.0);
